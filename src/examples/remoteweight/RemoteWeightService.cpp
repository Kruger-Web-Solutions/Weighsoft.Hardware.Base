...
void RemoteWeightService::loop() {
  ...
  if (now - _lastCleanupMillis >= REMOTE_WEIGHT_WS_CLEANUP_INTERVAL_MS) {
    ...
    // Trunk's WebSocketTxRx exposes cleanupClients()/countClients() but not the
    // raw AsyncWebSocket client list, so we can't flip each client to
    // close-on-queue-full here. cleanupClients() still reaps dead/closed
    // clients each tick; the 6 KB POST-drop guard above is the primary OOM
    // protection.
  ...
  if (now - _lastPeriodicMillis >= 30000UL) {
    ...
    Serial.printf("[RemoteWeight] heap=%lu min=%lu posts=%u dropped=%u ws_clients=%u\n",
                  ...
                  (unsigned)_webSocket.countClients());
  }
}
...
void RemoteWeightService::recordAudit(RemoteWeightAuditReason reason) {
  ...
  e.ws_clients = static_cast<uint8_t>(_webSocket.countClients());
...
void RemoteWeightService::readAuditAndStats(RemoteWeightService* self, JsonObject& root) {
  ...
  root["ws_clients"] = self->_webSocket.countClients();
...
#endif  // FT_ENABLED(FT_REMOTE_WEIGHT)
