...
- **`DISCONNECTED` with `len=15` or `len=22`:** The WebSockets library may pass a binary [RFC 6455](https://datatracker.ietf.org/doc/html/rfc6455#section-5.5.1) close frame **or** a plain **ASCII stack message** (e.g. `TCP connection cleanup` from lwIP). Firmware **0.7.7+** logs **`[ws-close] stack_msg text='...'`** for the latter so you do not mis-read `'T','C'` as a bogus numeric code (e.g. `21571`). For binary closes, see **`rfc6455_code`** / **`reason_hex`** in the runbook. Typical codes: `1000` normal closure, `1001` going away, `1006` abnormal (no close frame), `1008` policy violation, `1011` server error.
...
1. Capture **close payload bytes** for every `DISCONNECTED` — from firmware **0.7.6+** the writer logs **`[ws-close]`** (`no_close_payload`, **`stack_msg`** for ASCII TCP status from **0.7.7+**, or `rfc6455_code` / `reason_hex` for real close frames); see [SERIAL-FORWARDER-DISCONNECT-RUNBOOK.md](./SERIAL-FORWARDER-DISCONNECT-RUNBOOK.md).
